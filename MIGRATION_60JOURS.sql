-- ═══════════════════════════════════════════════════════════════════════════
-- MIGRATION_60JOURS.sql — Montage complet de la base sur une instance VIERGE
-- ═══════════════════════════════════════════════════════════════════════════
-- ORDRE D'EXÉCUTION : coller ce fichier ENTIER dans le SQL Editor de la nouvelle
-- instance Supabase, puis exécuter en UNE SEULE FOIS.
--
-- Contenu = les 42 migrations du repo concaténées dans l'ordre (tables, enum
-- app_role, fonctions, triggers, policies RLS, 3 buckets storage + policies,
-- extensions pg_cron/pg_net), avec UNIQUEMENT ces 5 modifications de seed :
--   1. Formation d'exemple « 90 jours en graphisme » : SUPPRIMÉE (aucune formation seedée)
--   2. 3 cohortes d'exemple : SUPPRIMÉES (aucune cohorte seedée)
--   3. site_settings : valeurs par défaut contenant « 90 » → vides (à remplir via l'admin)
--   4. contract_templates : template par défaut CONSERVÉ, « 90 jours/JOURS » → « 60 jours/JOURS »
--   5. formations.duration_days : DEFAULT 90 → 60
-- Aucune autre modification (structure, fonctions, policies inchangées).
-- ═══════════════════════════════════════════════════════════════════════════
...
-- Seed initial cohorts — SUPPRIMÉ (modif #2) : aucune cohorte d'exemple seedée.
-- La structure de la table cohorts reste intacte ; les cohortes seront créées via l'admin.
...
-- Seed with initial formation — SUPPRIMÉ (modif #1) : aucune formation d'exemple seedée.
-- La structure de la table formations reste intacte ; les formations seront créées via l'admin.
...
-- ════════════════════════════════════════
-- modif #5 : durée par défaut du programme 90 → 60 jours
ALTER TABLE public.formations ADD COLUMN duration_days integer NOT NULL DEFAULT 60;
-- ════════════════════════════════════════
...
<div class="container">
  ...
  <div class="sig-section">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:32px;margin-top:24px">
      ...
      <div>
        <div class="sig-line"></div>
        <div class="sig-label">Pour 60 Jours Formation</div>
  ...
  <div class="footer-note">
    Document généré automatiquement par la plateforme 60 Jours Formation. Conservez cet exemplaire comme preuve de votre engagement contractuel.
  </div>
</div>
</body>
</html>$TEMPLATE$,
