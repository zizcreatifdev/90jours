-- MIGRATION_CONTRAT_PREMIUM.sql
-- A exécuter dans l'éditeur SQL Supabase pour activer le nouveau contrat.
-- Met à jour le template générique actif (design premium + contenu rédigé +
-- variables {{frais_inscription}}, {{cout_total}}, {{livrable}}, {{date_signature}}).
-- Les contrats déjà signés ne sont pas affectés (snapshot figé).
