...
DECLARE
  ...
  notif_message TEXT;
BEGIN
  -- Briefs programmes (publish_at futur) : pas de notification a l'insertion.
  -- La notification sera declenchee cote application lors de la publication effective.
...
-- Fix 2 : RLS policy "Enrolled students can view briefs"
-- Les etudiants ne voient que les briefs dont publish_at <= now().
-- Les admins/staff ont une policy separee (FOR ALL) qui n'est pas touchee ici.
